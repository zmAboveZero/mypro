...
object PlateRandom {
  ...
  def getPalte: String = {
    ...
    while (count < 5) {
      ...
      val str2: String = b(random.nextInt(b.length))
      val num: Int = random.nextInt(10)
      var j = 0
      // 控制字母和数字的数量
      if (count > 2) {
        for (i <- 0 until  sb.length) {
          val c: Char = sb.charAt(i)
          if ((c >= 'A') && (c <= 'Z')) {
            j += 1
          }
        }
    ...
    val num1: Int = random1.nextInt(a.length)
    val plate: String = a(num1) + sb
    if (plate.matches("^.*[A-Z]{3}.*$")) getPalte
    else plate // 判断是否符合车牌规范，如果开头有连续的字母，则重新获取
  }

  def getModel: String = {
    val random1 = new Random()
    var num1: Int = random1.nextInt(c.length)
    if (num1 > 33) num1 = 23
    c(num1)
  }
  ...
  def main(args: Array[String]): Unit = {
    for (i <- 0 to 100) {
      System.out.println(PlateRandom.getPalte)
    }
  }
