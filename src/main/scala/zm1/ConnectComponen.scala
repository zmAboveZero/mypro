...
import org.apache.spark.graphx.lib.{ConnectedComponents, TriangleCount}
...
object ConnectComponen {
  def main(args: Array[String]): Unit = {
    ...
    val labelGraph = ConnectedComponents.run(graph)
    val vv = labelGraph.vertices.collect()
    vv.foreach(x => println("顶点ID:" + x._1 + " 子图:" + x._2))
