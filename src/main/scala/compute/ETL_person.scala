...
object ETL_person {
  def main(args: Array[String]): Unit = {
    var ss = SparkSession
      ...
      .master("local[2]")
      .getOrCreate()
    T_CRMS_D1_CUSTOMER.getTable(ss).createTempView("T_CRMS_D1_CUSTOMER")
    M6_BL_CVT_E02_CAR.getTable(ss).createTempView("M6_BL_CVT_E02_CAR")
    M6_AL_CF_RP_PERSONLOSSINFO.getTable(ss).createTempView("M6_AL_CF_RP_PERSONLOSSINFO")
    M6_AL_CF_RP_SETTLEMENTACCOUNT.getTable(ss).createTempView("M6_AL_CF_RP_SETTLEMENTACCOUNT")
    M6_AL_CF_RP_RELATEDPERSONNEL.getTable(ss).createTempView("M6_AL_CF_RP_RELATEDPERSONNEL")

    var T_CRMS_D1_CUSTOMER_df = ss.sql(
      """
        |select
        ...
        |from T_CRMS_D1_CUSTOMER
        |
      """.stripMargin)
    var M6_BL_CVT_E02_CAR_df = ss.sql(
      """
        |
        |select
        |'' as CUST_CODE	,
        ...
        |from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as etl_crt_tm,
        |from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as etl_upd_tm
        |from M6_BL_CVT_E02_CAR  t1 left join T_CRMS_D1_CUSTOMER t2
        |on t1.DRIVERNOTYPE=t2.CERT_TYPE and t1.DRIVERNO=t2.CERT_CODE
        |where  t2.CERT_TYPE=null
        |
  """.stripMargin)

    var M6_AL_CF_RP_PERSONLOSSINFO_df = ss.sql(
      """
        |select
        |'' as CUST_CODE,
        |'' as CUST_TYPE,
        ...
        |from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as etl_crt_tm ,
        |from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as etl_upd_tm
        |from M6_AL_CF_RP_PERSONLOSSINFO t1 left join T_CRMS_D1_CUSTOMER t2
    ...
    var M6_AL_CF_RP_SETTLEMENTACCOUNT_df = ss.sql(
      """
        |select
        |'' as CUST_CODE ,
        |'' as CUST_TYPE ,
        ...
        |from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as etl_crt_tm	,
        |from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as etl_upd_tm
        |from M6_AL_CF_RP_SETTLEMENTACCOUNT  t1 left join T_CRMS_D1_CUSTOMER t2
        |on t1.CERTIFICATETYPE=t2.CERT_TYPE and t1.CERTIFICATENO=t2.CERT_CODE
        |where t2.CERT_TYPE=null
        |
        |
    ...
    var M6_AL_CF_RP_RELATEDPERSONNEL_df = ss.sql(
      """
        |
        |select
        |'' as CUST_CODE ,
        ...
        |from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as etl_crt_tm ,
        |from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as etl_upd_tm
        |from M6_AL_CF_RP_RELATEDPERSONNEL t1 left join T_CRMS_D1_CUSTOMER t2
        |on t1.CERTIFICATETYPE=t2.CERT_TYPE and t1.CERTIFICATENO=t2.CERT_CODE
        |where t2.CERT_TYPE=null
        |
      """.stripMargin)

    var M6_AL_CF_RP_RELATEDPERSONNEL1_df = ss.sql(
      """
        |select
        |'' as CUST_CODE	,
        |t1.CUSTOMERTYPE as CUST_TYPE ,
    ...
    var union_df = T_CRMS_D1_CUSTOMER_df
      .union(M6_BL_CVT_E02_CAR_df)
      .union(M6_AL_CF_RP_PERSONLOSSINFO_df)
      .union(M6_AL_CF_RP_SETTLEMENTACCOUNT_df)
      .union(M6_AL_CF_RP_RELATEDPERSONNEL_df)
      .union(M6_AL_CF_RP_RELATEDPERSONNEL1_df)
